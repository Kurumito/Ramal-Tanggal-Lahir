...
function displayQuestion(index) {
    if (index < numberSets.length) {
        questionText.textContent = `Question ${index + 1} of ${numberSets.length}: Tanggal lahir kamu ada ga di set kotak ini?`;
...
function showResult() {
    ...
    setTimeout(() => {
        if (guessedNumber > 0 && guessedNumber <= 31) {
             resultText.textContent = `Tanggal lahir kamu adalah ${getOrdinal(guessedNumber)}! 🎉`;
        } else {
             // Handle the case where the user consistently answers "No" (guessedNumber = 0)
             if (guessedNumber === 0) {
                 resultText.textContent = "Kamu jawab 'No' pada semua set! Apakah kamu yakin kamu lagi mikir tanggal antara tanggal 1 dan 31? 🤔";
             } else {
                 resultText.textContent = "Hmm, ada yang salah, atau mungkin kamu enggak mikir hari di antara tanggal 1 dan 31? 🤔 Ayo kita coba lagi!";
             }
        }
